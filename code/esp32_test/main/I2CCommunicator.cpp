...
void interrupt_handler_to_read_i2c_message_from_master(uint8_t* buf,int numBytes);
...
I2CCommunicator::I2CCommunicator(){
	...
	i2c_driver_install(I2C_NUM, conf.mode, I2C_RX_BUF, I2C_TX_BUF, 0);

    // twi_setAddress(1);//Slave address
    // twi_attachSlaveTxEvent(interrupt_handler_to_respond_to_request_from_I2C_master);
    // twi_attachSlaveRxEvent(interrupt_handler_to_read_i2c_message_from_master);
    // twi_init();
}
